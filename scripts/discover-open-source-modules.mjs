...
const output = path.resolve(outputIndex >= 0 ? process.argv[outputIndex + 1] : path.join(projectDir, ".rack-web-cache", "open-source-modules.json"));
const catalog = JSON.parse(fs.readFileSync(path.join(projectDir, "public", "dynamic-plugins", "catalog.json"), "utf8"));
