...
function removePluginSource(plugin) {
  ...
  removedSourceBytes += directoryBytes(target);
...
for (const [plugin, items] of groups) {
 try {
  ...
  persist();
  process.stderr.write(`[${succeeded + failed + 1}/${candidates.length}] ${item.key}\n`);
  try {
...
console.log(JSON.stringify({ attempted: candidates.length, succeeded, failed, catalogModules: definitions.size, removedSourceBytes, statePath }, null, 2));
