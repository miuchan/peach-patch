...
export function importVcvPatch(
  ...
  const rack = Object.fromEntries(
    Object.entries(raw).filter(([key]) => key !== "modules" && key !== "cables"),
  );
  ...
  const cables = raw.cables.map((cable) => ({
    ...
    color: cable.color ?? cableColors[cable.id % cableColors.length],
    rack: { ...cable },
  }));
  return {
    modules,
