...
export function moduleFromDefinition(
    ...
    state:
      definition.key === "Stoermelder-P1/Stroke"
        ...
        : definition.stateKeys?.some((item) => item.default !== undefined)
          ? definition.stateKeys.map((item) => item.default ?? 0)
          : undefined,
...
export function sampleAssetFromData(data: Record<string, unknown> | undefined) {
  return assetFromValue(data?.patchworkWebAsset);
}
...
export function rackModifiersFromKeyboard(event: KeyboardEvent) {
  return (
    (event.shiftKey ? 1 : 0) |
    (event.ctrlKey ? 2 : 0) |
    (event.altKey ? 4 : 0) |
...
export function strokeBindings(module: ModuleInstance) {
  ...
  const values =
    module.state?.length || !data || typeof data !== "object"
      ? (module.state ?? [])
      : stateFromData(module.key, data as Record<string, unknown>, module.stateKeys);
  return Array.from({ length: 10 }, (_, id) => ({
    ...
    data:
      ...
      typeof ((data as Record<string, unknown>).keys as unknown[])[id] === "object"
        ? String(
            (((data as Record<string, unknown>).keys as unknown[])[id] as Record<string, unknown>)
              .data ?? "",
          )
        : "",
  }));
