...
struct json_t {
  enum class Type { Object, Array, Integer, Real, Boolean, String } type = Type::Object;
  double number = 0.0;
  std::string text;
...
inline void json_decref(json_t* value) { if (!value) return; if (value->type == json_t::Type::Object || value->type == json_t::Type::Array) for (json_t* child : value->values) json_decref(child); delete value; }
...
struct RackWebJsonReader {
  ...
  json_t* value() {
    ...
    if (end - cursor >= 4 && std::memcmp(cursor, "null", 4) == 0) { cursor += 4; return nullptr; }
...
struct int32_4 {
  ...
  explicit int32_4(const float_4& value) {
...
struct Input {
  ...
  Input() : voltages{}, connected(false) {}
...
struct Output {
  ...
  void setVoltage(float next, int channel = 0) { if (channel >= 0 && channel < 16) voltages[channel] = next; }
  void clearVoltages() { for (float& voltage : voltages) voltage = 0.f; }
  float* getVoltages(int firstChannel = 0) { return voltages + std::clamp(firstChannel, 0, 15); }
  template <typename T>
  T getVoltageSimd(int firstChannel = 0) const { T result{}; for (int lane = 0; lane < 4; lane++) result[lane] = getVoltage(firstChannel + lane); return result; }
  template <typename T>
  void setVoltageSimd(const T& value, int firstChannel = 0) { for (int lane = 0; lane < 4; lane++) setVoltage(value[lane], firstChannel + lane); }
  bool isConnected() const { return connected; }
  int getChannels() const { return channels; }
  void setChannels(int next) { channels = next < 0 ? 0 : (next > 16 ? 16 : next); }
...
// Rack exposes the input/output side through engine::Port::Type in cable
// connection events. Inputs and Outputs above intentionally keep their compact
// web-runtime storage, while this type preserves the source-compatible event API.
struct Port {
  enum Type {
    INPUT,
    OUTPUT,
...
struct ParamQuantity {
  ...
  virtual float getDisplayValue() { return 0.f; }
  virtual std::string getDisplayValueString() { return std::to_string(getDisplayValue()); }
  virtual std::string getUnit() { return unit; }
  virtual void setDisplayValueString(std::string next) { setDisplayValue(std::strtof(next.c_str(), nullptr)); }
  virtual void setDisplayValue(float next) { setValue(next); }
  virtual float getValue() const { return value; }
  virtual float getImmediateValue() const { return getValue(); }
  Param* getParam();
  const Param* getParam() const;
  virtual void setImmediateValue(float next) { setValue(next); }
  virtual void setValue(float next) { value = std::clamp(next, minValue, maxValue); }
...
struct Module {
  ...
  ParamQuantity* paramQuantities[256]{};
  ...
  // MIDI crosses the browser/real-time boundary as compact four-byte records:
  // [size, status, data1, data2]. Input adapters override rackWebPushMidi();
  // output adapters append records with rackWebEmitMidi() for the worklet to drain.
  ...
  virtual void rackWebPushMidi(int, int, int, int) {}
  ...
  ParamQuantity* configButton(int id, const char* name) { return configParam(id, 0.f, 1.f, 0.f, name); }
  ParamQuantity* configButton(int id, const std::string& name) { return configButton(id, name.c_str()); }
  ParamQuantity* configSwitch(int id, float minimum, float maximum, float initial, const char* name) { auto* quantity = configParam(id, minimum, maximum, initial, name); if (quantity) quantity->snapEnabled = true; return quantity; }
  ParamQuantity* configSwitch(int id, float minimum, float maximum, float initial, const std::string& name) { return configSwitch(id, minimum, maximum, initial, name.c_str()); }
  template <typename Labels>
  ParamQuantity* configSwitch(int id, float minimum, float maximum, float initial, const char* name, Labels&&) { return configSwitch(id, minimum, maximum, initial, name); }
  template <typename Labels>
  ParamQuantity* configSwitch(int id, float minimum, float maximum, float initial, const std::string& name, Labels&& labels) { return configSwitch(id, minimum, maximum, initial, name.c_str(), std::forward<Labels>(labels)); }
  ParamQuantity* configSwitch(int id, float minimum, float maximum, float initial, const char* name, std::initializer_list<const char*>) { return configSwitch(id, minimum, maximum, initial, name); }
  ParamQuantity* configSwitch(int id, float minimum, float maximum, float initial, const std::string& name, std::initializer_list<const char*> labels) { return configSwitch(id, minimum, maximum, initial, name.c_str(), labels); }
  template <typename Quantity>
  Quantity* configSwitch(int id, float minimum, float maximum, float initial, const char* name, std::initializer_list<const char*>) { auto* quantity = configParam<Quantity>(id, minimum, maximum, initial, name); if (quantity) quantity->snapEnabled = true; return quantity; }
  template <typename Quantity>
  ...
  Quantity* configSwitch(int id, float minimum, float maximum, float initial, const char* name, Labels&&) { auto* quantity = configParam<Quantity>(id, minimum, maximum, initial, name); if (quantity) quantity->snapEnabled = true; return quantity; }
  template <typename Quantity, typename Labels>
...
struct BooleanTrigger {
  enum State { Low, High, Uninitialized } state = Low;
  enum Event { NONE = 0, TRIGGERED = 1, UNTRIGGERED = -1 };
  bool process(bool next) { const bool triggered = state == Low && next; state = next ? High : Low; return triggered; }
  Event processEvent(bool next) { const Event event = state == Low && next ? TRIGGERED : state == High && !next ? UNTRIGGERED : NONE; state = next ? High : Low; return event; }
  void reset() { state = Low; }
  bool isHigh() const { return state == High; }
};
...
struct VuMeter2 {
  float lambda = 10.f;
  float level = 0.f;
  void process(float deltaTime, float value) { level = std::max(std::fabs(value), level * std::exp(-lambda * deltaTime)); }
  float getBrightness(float lowDb, float highDb) const {
    const float db = 20.f * std::log10(std::max(level, 1e-6f));
...
namespace random {
inline uint32_t state = 0x5a17c9e3u;
inline void init() {}
inline void seed(uint32_t next) { state = next ? next : 1u; }
inline uint32_t u32() { state ^= state << 13; state ^= state >> 17; state ^= state << 5; return state; }
inline float uniform() {
  return static_cast<float>(u32() >> 8) / static_cast<float>(1u << 24);
}
inline float normal() {
  const float first = std::fmax(uniform(), 1e-7f);
  const float second = uniform();
  return std::sqrt(-2.f * std::log(first)) * std::cos(6.28318530718f * second);
}
template <typename T>
T get() {
  if constexpr (std::is_same_v<T, bool>) return uniform() >= 0.5f;
  else if constexpr (std::is_floating_point_v<T>) return static_cast<T>(uniform());
  else return static_cast<T>(uniform() * static_cast<float>(~T(0)));
}
} // namespace random
