...
static bool rackWebFilesystemExists(const fs::path &) { return false; }
...
SurgeStorage::SurgeStorage(const SurgeStorage::SurgeStorageConfig &config) : otherscene_clients(0)
{
    ...
    if (!hasSuppliedDataPath)
    {
        ...
        if (rackWebFilesystemExists(userdp))
            datapath = userdp;
        else
            datapath = shareddp;
    }
    ...
    if (!hasSuppliedDataPath)
    {
        ...
        while (datapath.empty() && cp.has_parent_path() && cp != cp.parent_path())
        {
            ...
            if (rackWebFilesystemExists(portable))
            {
        ...
        if (!founddir)
        {
            ...
            {
                if (rackWebFilesystemExists(p) && !founddir)
                {
                    founddir = true;
                    datapath = p;
                }
    ...
    if (!hasSuppliedDataPath)
    {
        ...
        if (datapath.empty() || !rackWebFilesystemExists(datapath))
        {
    ...
    try
    {
        ...
        if (rackWebFilesystemExists(userDataPath))
        {
    ...
    if (rackWebFilesystemExists(datapath / "configuration.xml"))
    {
...
#else
    if (rackWebFilesystemExists(datapath / "windows.wt"))
    {
...
void SurgeStorage::createUserDirectory()
{
    ...
    if (!rackWebFilesystemExists(p) || !rackWebFilesystemExists(userPatchesPath))
    {
        needToBuild = true;
    }
    ...
    if (userDataPathValid)
    {
        ...
        if (!rackWebFilesystemExists(userPatchesMidiProgramChangePath))
        {
            try
            {
                                              ...
                                              std::vector<PatchCategory> &categories)
{
    ...
    try
    {
        ...
        if (!rackWebFilesystemExists(patchpath))
        {
            return;
        }
...
void SurgeStorage::refresh_wtlist()
{
    ...
    if (extraThirdPartyWavetablesPath.empty() ||
        !rackWebFilesystemExists(extraThirdPartyWavetablesPath / "wavetables_3rdparty"))
    {
