...
The browser loads the mutable registry index over HTTPS and verifies each immutable WASM artifact by its declared size and SHA-256 digest before instantiation. Source discovery, Rack compatibility adaptation, Emscripten compilation, artifact publication, and the source-build regression suite live in the companion Peach Patch Registry repository. This checkout contains only the runtime client and the small Rack UI component assets required to render the browser shell.
