...
The authoritative reference is the Rack `v2.6.6` tree in `Rack/`. A module is marked `WASM READY` only when a compiled artifact implements the same ordered parameter, input, output, and light indices as its Rack model and its documented DSP contract has an executable regression test. Library metadata alone never marks a module ready. Translations that replace SIMD/native DSP with browser-specific algorithms say so in the registry description.

## Why desktop plugin binaries cannot be loaded directly

Rack Library `.vcvplugin` packages contain a native shared library for one OS/CPU target. Browser WebAssembly cannot load Mach-O, ELF, or PE machine code. The runtime therefore uses official Library metadata and open-source repositories. It loads a bundled WASM artifact when one exists, or asks the localhost-only builder to isolate and compile a compatible DSP class from source.
...
4. The `(plugin, model)` key is checked against the bundled registry and the dynamic local catalog, but an explicit URL still attempts the official-source compiler first. A bundled artifact is retained as the fallback if compilation reports unsupported host APIs.
   Once a dynamic exact-source definition exists, it replaces the same-key authored fallback in both direct lookup and every enumerated module list, including Library, Quick Add, hydration, and replacement flows.
5. `scripts/plugin-builder-server.mjs` fetches the official manifest and resolves an immutable source revision. It uses the plugin gitlink commit from the official Library `v2` index; official VCV repositories that are not submodules use the exact `v<manifest.version>` tag. Recursively declared source submodules are initialized only from validated HTTPS GitHub URLs, safe relative paths, and the exact `160000` gitlink commits recorded by each parent checkout. A validated local index is reused if the refresh endpoint is temporarily unavailable. The checked-out `plugin.json` slug and version must match before scanning begins.
6. The scanner finds the exact `createModel<Module, Widget>()` registration, isolates the module class, transitive plugin-defined module bases, referenced cross-file DSP dependencies, and brace-balanced DSP preludes, and excludes widget/native UI code. Compatible classes are wrapped in ABI 0.3 and compiled with Emscripten. Out-of-line and nested methods, typed nested JSON arrays, sample-rate callbacks, and the supported scalarized `simd::float_4` subset are preserved. Parameter defaults are then read from the initialized WASM instance so constructor-local expressions are authoritative. The resulting WASM and inferred controls/ports are registered immediately.
7. Unsupported Rack APIs produce `SOURCE BUILD NEEDED` with `manual-browser-adapter`; they are never marked ready from metadata alone.
...
`lib/web-plugin-registry.ts` is the single authored build inventory for bundled Web ABI modules. It generates `web-runtime/modules.json`; each generated record binds a Library key to one C++ entry, output artifact, initial WASM memory size, and one of three declared compatibility strategies:
...
`scripts/generate-web-runtime-manifest.mjs` emits the JSON artifact, and `scripts/read-web-runtime-manifest.mjs` validates keys, identifiers, memory alignment, uniqueness, and C++ entry existence before compilation. `scripts/build-web-runtime.sh` accepts zero or more exact Rack keys. The test suite enforces exact generated output and valid ABI exports.

The dynamic pipeline uses `scripts/scaffold-library-module.mjs`. It supports ordinary, template-specialized, and macro-configured shared-header `createModel<Module, Widget>()` registrations, classic `ParamIds/NUM_PARAMS` and Rack 2.6 `ParamId/PARAMS_LEN` conventions, explicit enum assignments, zero-light classic modules without `LightIds`, arithmetic or source/template-constant `ENUMS()` counts, same-file DSP preludes, transitive plugin module bases, safe original DSP headers plus referenced cross-file implementations, recursively locked Git submodules, uniquely resolved headers in nested repositories, and mixed GNU C/Rack C++ source families. Official source locking follows the `.gitmodules` `path` and validated repository URL rather than assuming its human-readable submodule label equals the plugin slug, and a no-checkout clone is always populated even when the requested gitlink is already the remote default `HEAD`. Object- and function-like DSP macros, simple preprocessor feature branches, C-style numeric casts, and formatted `rack::string::f()` port names are retained. Inline and out-of-line/nested DSP methods and constructors with initializer lists, alias-driven template implementation closure, conditionally compiled DSP implementation units, namespace globals referenced by selected implementations, recursively referenced free helper functions, trigger edge events, lifecycle/randomize events, editable parameter strings and dynamic switch labels, timers/clock dividers/meters/slew limiters/exponential filters, vector-compatible module ports, polyphonic planar buffers, bypass routes, scalar/nested-array state and finite JSON string enums, seeded random sources, ring buffers, RC/IIR filters, canonical-layout radix-2 real FFT, Rack's MinBLEP construction flow, stateful sample-rate conversion, and `crossfade()` are preserved. `Timer::process()` follows Rack 2.6's elapsed-time return contract as well as its state mutation, and plugins that include Rack's public `<math.hpp>` directly receive the same browser-safe namespace API. When a constructor omits `configInput()` or `configOutput()`, exact enum identifiers become readable port labels with separated numeric channels and inform gate/audio/CV classification, including conventional left/right stereo audio names. Message-expander metadata requires real target-class access to producer/consumer buffers, neighbor modules, or flip requests; unrelated model declarations in shared plugin headers do not create false adjacency groups. Configuration calls in out-of-line constructors now provide the same names, ranges, port labels, and bypass routes as inline constructors. Mixed helper classes retain their DSP/state members while methods that inspect the native Rack scene are replaced with inert browser stubs; native `MenuItem`, `ui::Menu`, `SvgPanel`, and event declarations are removed. UI-symbol filtering considers declaration headers rather than method bodies, so module constructors containing `configSwitch()` or custom switch quantities are not mistaken for native widget code. Comment-aware brace matching prevents implementation extraction from being truncated by source comments, and local dependency headers are topologically ordered before inlining. Static `Vec`/`mm2px(Vec)` widget calls and named `Vec` members are recovered as official browser control/jack coordinates. Nearby local widget headers contribute finite scalar and `std::array` layout constants; `RACK_GRID_WIDTH`/`RACK_GRID_HEIGHT`, `box.size.x/y`, scalar `mm2px()` calls, range-for initializer lists, and their explicit incremented column counters are evaluated without scanning unrelated DSP dependencies. Its scalar four-lane layer covers float and integer lanes, arithmetic, comparisons, lane-select/logical masks, four-value construction, SSE-style lane shuffle/move operations, load/store, clamp, min/max, sign, absolute value, modulo, square root, power, exponent/logarithm, sine/cosine, complex magnitude/argument, floor/round/truncation, conditional selection, movemask/inverse masks, and polyphonic normalled inputs. Older modules that directly expose `__m128`, `__m128d`, or `__m64` now compile with Emscripten's SSE/SSE2 compatibility headers and `-msimd128`; Rack `float_4::v` interoperates with the native vector type, the four omitted MMX mask helpers have portable equivalents, and legacy mathfun exp/log calls retain lane behavior. `BINARY()` ROM declarations are converted from locked `.bin` assets into aligned WebAssembly data, so factory wavetables are not silently approximated. Memory starts page-aligned with 4 MiB constructor headroom and automatically doubles after either a link-time static-data failure or a constructor memory trap, capped at 256 MiB; an explicit memory setting remains strict. Minimal WASI `fd_write` acknowledgement and deterministic `random_get` keep diagnostic- and entropy-enabled modules browser-loadable. After compilation, parameter defaults and min/max ranges are introspected from the real constructor instead of trusting partial source expressions. Verified checkout directories whose names equal their Git HEAD provide an offline fallback when the partial Library index cannot fetch a missing blob. Unknown SIMD symbols remain manual-adapter work instead of being compiled optimistically. Bruer/SEQ1 compiled from its original class matches the bundled translation across 12,288 sampled outputs. Bidoo `lATe`, `dTrOY`, `BanCau`, and `lIMbO` 2.1.1 compile from exact revision `85c00f2aefa22d72d2a7472a1a937a962be3b07d`; their enum-derived clock/CV/gate/audio ports, absence of a false expander contract, lATe's original 1 ms 10 V pulse, dTrOY's external-clock sequencer advance through its quantizer, BanCau's dual-envelope routing, and lIMbO's independent stereo ladder state are permanent regressions.
