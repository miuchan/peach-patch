...
class RackPluginProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = async ({ data }) => {
      if (data.type === "load") {
        ...
        if (
          data.assets &&
          this.runtime.rack_web_asset_capacity_for_slot &&
          this.runtime.rack_web_asset_buffer_for_slot &&
          this.runtime.rack_web_commit_asset_for_slot
        ) {
          for (let slot = 0; slot < data.assets.length; slot++) {
            ...
            new Float32Array(
              ...
              this.runtime.rack_web_asset_buffer_for_slot(slot),
              length,
        ...
        if (
          data.asset &&
          this.runtime.rack_web_asset_capacity &&
          this.runtime.rack_web_asset_buffer &&
          this.runtime.rack_web_commit_asset
        ) {
          const capacity = this.runtime.rack_web_asset_capacity();
          const samples =
            data.asset.samples instanceof Float32Array
              ? data.asset.samples
              : new Float32Array(data.asset.samples);
          const length = Math.min(capacity, samples.length);
          new Float32Array(
            this.runtime.memory.buffer,
            this.runtime.rack_web_asset_buffer(),
          ...
          const frames = Math.min(
            data.asset.frames,
            Math.floor(length / Math.max(1, data.asset.channels)),
          );
          this.runtime.rack_web_commit_asset(frames, data.asset.channels, data.asset.sampleRate);
        }
        this.params = data.params || [];
        for (let id = 0; id < (data.state || []).length; id++)
          this.runtime.rack_web_set_state(id, data.state[id]);
  ...
  process(inputs, outputs) {
    ...
    for (let port = 0; port < outputCount; port++)
      runtime.rack_web_set_output_connected(port, this.outputConnections[port] ? 1 : 0);
    const rackInputs = new Float32Array(
    ...
    for (let port = 0; port < inputCount; port++) {
      const bus = inputs[port] || [],
        channels = Math.min(maxChannels, bus.length || (this.inputConnections[port] ? 1 : 0));
      ...
      for (let channel = 0; channel < Math.min(channels, bus.length); channel++)
        for (let frame = 0; frame < frames; frame++)
          rackInputs[(channel * inputCount + port) * 128 + frame] = bus[channel][frame] * 5;
    }
    runtime.rack_web_process(frames, sampleRate);
    const rackOutputs = new Float32Array(
      runtime.memory.buffer,
    ...
    for (let port = 0; port < Math.min(outputCount, outputs.length); port++) {
      ...
      for (let channel = 0; channel < outputs[port].length; channel++) {
        const destination = outputs[port][channel];
        if (channel >= activeChannels) {
          destination.fill(0);
          continue;
        }
        for (let frame = 0; frame < frames; frame++)
          destination[frame] = rackOutputs[(channel * outputCount + port) * 128 + frame] / 5;
      }
    }
    return true;
  }
