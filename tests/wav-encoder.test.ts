...
test("WAV encoder writes a standards-compliant stereo PCM header", async () => {
  const pcm = floatPcm16Part(new Float32Array([-2, -1, -0.5, 0, 0.5, 1, 2, 0]));
  const blob = createWavBlob([pcm], 4, 2, 48000),
    bytes = await blob.arrayBuffer(),
    view = new DataView(bytes),
    text = (offset: number, length: number) =>
      String.fromCharCode(...new Uint8Array(bytes, offset, length));
  assert.equal(blob.type, "audio/wav");
  assert.equal(blob.size, 60);
  assert.equal(text(0, 4), "RIFF");
  assert.equal(view.getUint32(4, true), 52);
