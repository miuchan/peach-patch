...
test("Rack parameter double-click requires the same control and pointer type", () => {
  ...
  assert.equal(
    registerRackParamPress(first.next, 4, "mouse", 2_001).doubleClick,
    false,
  );
});
...
test("double-click reset can follow native dynamic default quantities", () => {
  ...
  assert.equal(
    rackParamResetValue(
      { id: 2, name: "Follower", min: 1, max: 0, default: 0.5, resetFrom: { paramId: 1, scale: 1, offset: 0 } },
      values,
    ),
    0.35,
  );
  assert.equal(
    rackParamResetValue(
      { id: 3, name: "Curve", min: 0, max: 1.3, default: 0.75, resetFrom: { paramId: 2, scale: 0.5, offset: 0.5 } },
      values,
    ),
    0.9,
  );
