...
test("the graph worklet restores nested VCV module JSON before audio starts",async()=>{
  ...
  try{
    execFileSync(process.execPath,[path.join(root,"scripts","scaffold-library-module.ts"),"https://library.vcvrack.com/FixturePlugin/HostMixed","--manifest-file",path.join(fixtureSource,"plugin.json"),"--source-dir",fixtureSource,"--output",temporary,"--compile"],{stdio:"pipe"});
