function decimalText(value:number){
  ...
  if(absolute>=.9975)return"   1";
  if(absolute<.005)return"   0";
...
function displayText({
  ...
  if(notifiedParam===randomSemiParam)return`  ${String(Math.abs(Math.round(params[randomSemiParam]??0))).padStart(2," ")}`;
  const delay=Math.round(params[tapParam]??0);
  if(delay<1)return"  - ";
  const activeTaps=[0,1,2,3].filter(index=>(params[index]??0)>=.5).length,
