...
const integer = (value: number | undefined, fallback = 0) =>
  Number.isFinite(value) ? Math.round(value as number) : fallback;
const characters = (values: number[] | undefined, offset: number) =>
  Array.from({ length: 4 }, (_, index) =>
    String.fromCharCode(clamp(integer(values?.[offset + index], 32), 0, 255)),
  ).join("");
...
export function RackWolframDisplay({
  ...
  const menuActive = (values?.[0] ?? 0) > 0.5,
    ...
    textBackgroundPadding = fontSize * 0.5 - textBackgroundSize * 0.5 + padding,
    wolfSeedSize = fontSize * 0.5 - 2,
    wolfSeedY = fontSize * 0.5 * 4 + 2,
    firstMatrixRow = miniMenuActive && !menuActive ? cells - 4 : 0,
    matrix = Array.from({ length: cells * cells }, (_, index) => (values?.[29 + index] ?? 0) > 0.5),
    menuLines =
      page === 0
        ? [
            page < 2 ? engineLabel.toLowerCase() : "menu",
            "SEED",
            selected === 0 ? (seed === 256 ? "RAND" : "    ") : seedLabel,
            "<#@>",
          ]
        : page === 1
    ...
    drawTextBackground = (row: number) => (
      <g key={`text-bg-${row}`}>
        {Array.from({ length: 4 }, (_, column) => (
          <rect
            key={column}
            x={column * fontSize + textBackgroundPadding}
            y={row * fontSize + textBackgroundPadding}
            width={textBackgroundSize}
            height={textBackgroundSize}
            rx="3"
    ...
    drawWolfSeed = (foreground: boolean) => (
      <g key={foreground ? "wolf-seed-on" : "wolf-seed-off"}>
        {Array.from({ length: 8 }, (_, column) => {
          const on = Boolean((seed >> (7 - column)) & 1);
          return on === foreground ? (
            <rect
              key={column}
              x={column * fontSize * 0.5 + 2}
              y={wolfSeedY}
              width={wolfSeedSize}
              height={wolfSeedSize * 2 + 2}
              rx="3"
              fill={foreground ? style.foreground : style.background}
            />
          ) : null;
        })}
        {foreground &&
          Array.from({ length: 7 }, (_, index) => {
            const column = index + 1,
              xPosition = column * fontSize * 0.5 + 1;
            return (
              <path
                key={column}
                d={`M ${xPosition} ${wolfSeedY - 1} v 2 M ${xPosition} ${wolfSeedY + fontSize - textBackgroundPadding - 1} v 2`}
                fill="none"
    ...
    >
      ...
      {menuActive && (
        <>
          {Array.from({ length: 4 }, (_, row) =>
            page === 0 && selected === 0 && seed !== 256 && row === 2
              ? drawWolfSeed(false)
              : drawTextBackground(row),
          )}
          {page === 0 && selected === 0 && seed !== 256 && drawWolfSeed(true)}
          {menuLines.map((line, row) => (
            <text
              key={row}
              x="1"
              y={1 + fontSize * row}
              fill={style.foreground}
              fontFamily={'"Modular Mooch Wolfram", monospace'}
              fontSize={fontSize}
      ...
      {!menuActive &&
        matrix.map((on, index) => {
          ...
          if (cellStyle === 1) {
            const inset = (cellPadding - roundedSize) * 0.5 + padding;
            return (
              <rect
                key={index}
                x={column * cellPadding + inset}
                y={row * cellPadding + inset}
                width={roundedSize}
                height={roundedSize}
                rx="1"
                fill={on ? style.foreground : style.background}
              />
            );
          }
          const center = cellPadding * 0.5 + padding;
          return (
            <circle
              key={index}
              cx={column * cellPadding + center}
              cy={row * cellPadding + center}
              r={circleRadius}
              fill={on ? style.foreground : style.background}
            />
          );
