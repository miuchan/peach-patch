...
export function RackMadzineManual({
  ...
  const [query,setQuery]=useState(""),[selectedSlug,setSelectedSlug]=useState<string|null>(null),
    ...
    modules=useMemo(()=>Object.entries(help).sort((left,right)=>left[1].name.localeCompare(right[1].name)),[help]),
    matches=useMemo(()=>{
      const needle=query.trim().toLocaleLowerCase();
      if(!needle)return modules;
      return modules.filter(([slug,module])=>
        `${slug} ${module.name} ${module.description[language]} ${module.entries.map(entry=>entry.name).join(" ")}`.toLocaleLowerCase().includes(needle));
    ...
    activeEntry=activeTargetName&&active
      ? active.entries.find(entry=>entry.name.localeCompare(activeTargetName,undefined,{sensitivity:"accent"})===0)
      : undefined,
    ...
    controlStyle=(left:number,top:number,width:number,height:number)=>({
      left:left*scaleX,top,width:width*scaleX,height,
    });

  const updateFont=(delta:number)=>onData({fontSize:Math.max(8,Math.min(32,fontSize+delta))});
  return <div className="pw-madzine-manual" aria-label="MADZINE Manual">
    <button
      type="button"
      ...
      onDoubleClick={(event)=>{stopDouble(event);onData({language:1})}}
    ...
    <button
      ...
      onDoubleClick={(event)=>{stopDouble(event);onData({fontSize:20})}}
    >A−</button>
    <button
      type="button"
      className="pw-madzine-manual-control font"
    ...
    >
      {active ? <>
        ...
        {target?.targetName&&<strong className={target.targetType??""} style={{fontSize,lineHeight:1.4}}>{target.targetName}</strong>}
        <hr/>
        <p style={{fontSize,lineHeight:1.4}}>{body}</p>
        {!target?.targetName&&active.entries.length>0&&
          <dl className="pw-madzine-manual-entries">
            {active.entries.map(entry=><div key={entry.name}>
              <dt>{entry.name}</dt>
        ...
        <input
          ...
          placeholder={language==="zh"?"搜尋模組或控制項":language==="ja"?"モジュールを検索":"Search modules or controls"}
          value={query}
          onChange={event=>setQuery(event.target.value)}
          onDoubleClick={stopDouble}
        />
        <div className="pw-madzine-manual-directory" role="list">
          {matches.map(([slug,module])=><button key={slug} type="button" role="listitem" onClick={()=>setSelectedSlug(slug)}>
            <b>{module.name}</b>
            <span>{module.description[language]}</span>
          </button>)}
          {!matches.length&&<p className="empty">{language==="zh"?"沒有結果":language==="ja"?"結果なし":"No matches"}</p>}
        </div>
      </>}
    </section>
  </div>;
