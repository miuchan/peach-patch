...
export const RackUndertowPreview = memo(function RackUndertowPreview({
  ...
  useEffect(() => {
    ...
    const pad = 1.625,
      left = pad,
      right = Math.max(left + 1, displayWidth - pad),
      top = pad,
      bottom = Math.max(top + 1, height - pad),
      ...
      points = Array.from({ length: POINTS }, (_, index) => {
        ...
        return {
          x: left + (index / (POINTS - 1)) * drawWidth,
          y: top + (0.5 - 0.1 * voltage) * drawHeight,
        };
      }),
      now = performance.now(),
      tracerEnabled = (values?.[5] ?? 1) > 0.5,
      previous = previousRef.current,
      changed = Boolean(
        previous?.some((point, index) => Math.abs(point.y - points[index].y) > 0.02),
      );
    if (tracerEnabled && previous && changed)
      trailsRef.current.push({ born: now, points: previous });
    ...
    context.beginPath();
    context.moveTo(left, top + 0.5 * drawHeight);
    context.lineTo(right, top + 0.5 * drawHeight);
    context.stroke();
    context.lineCap = "butt";
    context.lineJoin = "round";
    context.lineWidth = 1.05;
    for (const trail of trailsRef.current) {
      context.strokeStyle = `rgba(255,190,80,${Math.max(0, 1 - (now - trail.born) / TRAIL_FADE_MS) * 0.41})`;
      stroke(
        context,
        trail.points.filter((_, index) => index % 2 === 0 || index === POINTS - 1),
      );
    }
