...
function noteText(voltage: number, flats: boolean) {
  ...
  if (Math.abs(deviation) < 0.01) return `${name}${octave}`;
...
export function RackNoteMeterDisplay({
  samples,
  params,
  x,
  y,
  ...
  const mode = Math.max(0, Math.min(2, Math.round(params[1] ?? 0))),
    ...
    color =
      COLORS[Math.max(0, Math.min(COLORS.length - 1, Math.round(params[3] ?? 0)))] ?? COLORS[0],
