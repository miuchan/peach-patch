...
function speedToKnob(speed: number) {
  ...
  if (speed < 1) return 0.25 + speed / 4;
  return 0.5 + (speed - 1) / 14;
}
...
export function RackMadzineLaunchpad({
  ...
  const release = (event: PointerEvent<HTMLButtonElement>) => {
    ...
    if (!drag.held) {
      if (drag.target !== drag.source)
        action((drag.copy ? 5300 : 1200) + drag.source * cellCount + drag.target);
  ...
  return (
    <div
      ref={gridRef}
      className="pw-madzine-launchpad"
      style={{ left: x * scaleX, top: y, width: width * scaleX, height }}
    >
      {Array.from({ length: cellCount }, (_, cell) => {
        const row = Math.floor(cell / columns),
          column = cell % columns,
          state = Math.max(0, Math.min(6, Math.round(visual[cell * 4] ?? 0))),
          loopClocks = Math.max(0, Math.round(visual[cell * 4 + 1] ?? 0)),
          progress = Math.max(0, Math.min(1, visual[cell * 4 + 2] ?? 0)),
          wave = visual.slice(waveOffset + cell * wavePoints, waveOffset + (cell + 1) * wavePoints),
          maximum = Math.max(0.001, ...wave.map((value) => Math.abs(value))),
          points = wave
            .map(
              (value, index) =>
                `${4 + (index / (wavePoints - 1)) * (cellWidth - 8)},${cellHeight / 2 - (value / maximum) * (cellHeight / 2 - 4)}`,
            )
            .join(" ");
        return (
          <button
            ...
            style={{
              ...
              height: cellHeight,
              backgroundColor: cellColors[state],
            }}
            onPointerDown={(event) => {
              ...
              holdTimerRef.current = window.setTimeout(() => {
                const drag = dragRef.current;
                if (!drag || drag.source !== cell || drag.target !== cell) return;
                drag.held = true;
                action(1100 + cell);
              }, 1000);
            }}
          ...
          >
            {(state === 2 || state === 5) && wave.some((value) => Math.abs(value) > 0.001) && (
              <svg
              ...
              >
                ...
                <line
                  x1={4 + progress * (cellWidth - 8)}
                  x2={4 + progress * (cellWidth - 8)}
                  y1="2"
                  y2={cellHeight - 2}
                />
              </svg>
      ...
      {speedCell !== null &&
        (() => {
          const speed = visual[speedCell * 4 + 3] ?? 1,
            knob = Math.max(0, Math.min(1, speedToKnob(speed))),
            column = speedCell % columns,
            row = Math.floor(speedCell / columns);
          return (
            <div
              className="pw-launchpad-speed"
              style={{
                left: Math.min(width * scaleX - 156, column * spacingX * scaleX),
                top: Math.min(height - 74, row * spacingY + cellHeight),
              }}
            >
              <b>
              ...
              <input
                ...
                min="0"
                max="1000"
                defaultValue={Math.round(knob * 1000)}
                onPointerDown={(event) => event.stopPropagation()}
                onDoubleClick={(event) => {
                  event.preventDefault();
                  event.stopPropagation();
                  event.currentTarget.value = "500";
                  setSpeed(speedCell, 0.5);
                }}
