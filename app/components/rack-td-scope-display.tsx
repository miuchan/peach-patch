...
function brightened(color: readonly number[], brightness: number) {
  const scale = brightness <= 0.5 ? 0.35 + (brightness / 0.5) * 0.65 : 1;
  const lift = brightness <= 0.5 ? 0 : ((brightness - 0.5) / 0.5) * 0.42;
  return color.map((channel) =>
    Math.round(Math.min(255, channel * scale + (255 - channel * scale) * lift)),
  );
}
...
export const RackTdScopeDisplay = memo(function RackTdScopeDisplay({
  ...
  useEffect(() => {
    ...
    const linked = (values?.[0] ?? 0) > 0.5;
    const preview = (values?.[1] ?? 0) > 0.5;
    if (!linked || !preview) {
      context.strokeStyle = "rgba(122,92,255,.48)";
      context.lineWidth = 1.25;
      context.beginPath();
      context.moveTo(displayWidth * 0.16, height * 0.72);
    ...
    for (let row = 0; row < rows; row++) {
      ...
      const yPosition = ((row + 0.5) * height) / rows;
      for (let channel = 0; channel < (stereo ? 2 : 1); channel++) {
        ...
        const half = laneWidth * 0.46;
        context.strokeStyle = `rgba(${color.join(",")},${0.32 + peak * 0.68})`;
        context.lineWidth = Math.max(0.55, (height / rows) * 0.7);
        context.beginPath();
    ...
    let marker = Math.max(0, Math.min(1, values?.[4] ?? 0.5));
    const sampleMode = ((values?.[2] ?? 0) & 1) !== 0;
    marker = sampleMode ? 0.5 : Math.max(0.5, marker);
    if (inverted) marker = 1 - marker;
    context.strokeStyle = "rgba(238,244,247,.72)";
    context.lineWidth = 0.75;
    context.beginPath();
    context.moveTo(0, marker * height + 0.5);
    context.lineTo(displayWidth, marker * height + 0.5);
    context.stroke();
