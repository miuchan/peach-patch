...
export function RackSegmentDisplay({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      context.lineWidth = 1;
      context.beginPath();
      context.roundRect(0, 0, displayWidth, height, 5);
      context.fill();
      context.stroke();
      context.font = `${38 * scaleX}px BraidsSegment14, monospace`;
      context.textBaseline = "alphabetic";
      context.fillStyle = "rgba(175,210,44,.063)";
      context.fillText("~~~~", 9 * scaleX, 48);
      const index = Math.max(
        0,
        Math.min(values.length - 1, Math.round(value * (values.length - 1))),
      );
      context.fillStyle = "#afd22c";
      context.shadowColor = "#afd22c";
      context.shadowBlur = 2;
      context.fillText(values[index] ?? values[0] ?? "", 9 * scaleX, 48);
    };
    const font = new FontFace("BraidsSegment14", `url(${segmentFontUrl})`);
    font
      .load()
      .then((loaded) => {
