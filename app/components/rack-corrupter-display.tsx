...
export function RackCorrupterDisplay({
  ...
  const writePosition = Math.max(0, Math.min(bins - 1, Math.round(values?.[0] ?? 0))),
    ...
    status = [
      {
        label: "BND",
        active: (values?.[2] ?? 0) > 0.5,
        x: ((17 - 7) / 77.44) * width,
        color: "#00ff80",
      },
      {
        label: "BRK",
        active: (values?.[3] ?? 0) > 0.5,
        x: ((35.48 - 7) / 77.44) * width,
        color: "#ff8000",
      },
      {
        label: "FRZ",
        active: (values?.[4] ?? 0) > 0.5,
        x: ((53.96 - 7) / 77.44) * width,
        color: "#4080ff",
      },
    ];
  return (
    ...
    >
      ...
      {Array.from({ length: bins }, (_, bin) => {
        ...
        return (
          <rect
            ...
            x={bin * columnWidth}
            y={middle - barHeight}
            width={Math.max(columnWidth, 0.35)}
            height={barHeight * 2}
      ...
      <rect
        x={(writePosition / bins) * width}
        y="0"
        width="1.5"
        height={waveHeight}
        fill="rgba(255,255,255,.706)"
      />
      {status.map(
        (item) =>
          item.active && (
