...
export const RackTemporalDeckDisplay = memo(function RackTemporalDeckDisplay({
  ...
  const cy = height / 2;
  const radius = Math.min(displayWidth, height) * 0.42;
  const angle = values?.[offset] ?? 0;
  const playhead = Math.max(0, values?.[offset + 1] ?? 0);
  const frames = Math.max(0, values?.[offset + 2] ?? 0);
  const sampleMode = (values?.[offset + 3] ?? 0) > 0.5;
  const loaded = (values?.[offset + 4] ?? 0) > 0.5;
  const playing = (values?.[offset + 5] ?? 0) > 0.5;
    ...
    >
      <svg width={displayWidth} height={height} viewBox={`0 0 ${displayWidth} ${height}`}>
        ...
        {Array.from({ length: 31 }, (_, index) => {
          ...
          const color =
            red > 0.05
              ? `rgba(255,46,51,${Math.max(0.14, red)})`
              : `rgba(255,199,45,${Math.max(0.1, yellow)})`;
          return (
            <g key={index}>
              <circle
                cx={lx}
                ...
                r={2.6}
                fill={color}
                filter={(red || yellow) > 0.45 ? "url(#temporal-deck-glow)" : undefined}
              />
              <circle cx={lx} cy={ly} r={1.25} fill={color} />
            </g>
          );
        })}
        <g style={{ transformOrigin: `${cx}px ${cy}px`, transform: `rotate(${angle}rad)` }}>
          ...
          {Array.from({ length: 11 }, (_, index) => (
            <circle
              ...
              cx={cx}
              cy={cy}
              r={radius * (0.35 + index * 0.055)}
              fill="none"
              stroke="rgba(145,177,190,.12)"
              strokeWidth=".45"
            />
          ))}
          <path
            d={`M ${cx} ${cy - radius * 0.93} L ${cx + 4} ${cy - radius * 0.72} L ${cx - 4} ${cy - radius * 0.72} Z`}
            fill="#00c6e4"
          />
          <circle
            cx={cx}
        ...
        <g
          transform={`rotate(${24 + (sampleMode && frames ? Math.min(32, (playhead / frames) * 32) : 0)} ${cx + radius * 0.87} ${cy - radius * 0.58})`}
        >
          <circle
            cx={cx + radius * 0.87}
            cy={cy - radius * 0.58}
          ...
          />
          <path
            d={`M ${cx + radius * 0.87} ${cy - radius * 0.58} L ${cx + radius * 0.2} ${cy + radius * 0.55}`}
            stroke="#aeb6ba"
            strokeWidth="4"
            strokeLinecap="round"
          />
          <path
            d={`M ${cx + radius * 0.23} ${cy + radius * 0.51} l -8 9`}
            stroke="#efc768"
            strokeWidth="6"
            strokeLinecap="round"
          />
        ...
        <text
          x={cx}
          y={cy + radius * 0.55 + 8.5}
          textAnchor="middle"
          fontSize="6.5"
          fontFamily="ui-monospace, monospace"
          fill={loaded ? "#56e0a4" : "#7ba1ad"}
        >
          {loaded
            ? `${playing ? "PLAY" : "HOLD"} · ${CARTRIDGES[cartridge]}`
            : `LIVE · ${CARTRIDGES[cartridge]}`}
        </text>
      </svg>
    </div>
  );
