...
export function RackStudioInspector({
  ...
  const visibleParams = definition.params.filter((param) => !param.hidden && !param.button);
  const stateKeys = definition.stateKeys ?? [];
  ...
  return (
    <aside className="pw-inspector" aria-label={`Live inspector for ${module.plugin}/${module.model}`}>
      ...
      {definition.params.length > 0 && <div className="pw-midi-learn"><label><span>MIDI LEARN TARGET</span><select aria-label={`${module.model} MIDI learn parameter`} value={selectedLearnParamId} onChange={(event) => setMidiLearnParamId(Number(event.target.value))}>{definition.params.map((param) => <option key={param.id} value={param.id}>{param.name}</option>)}</select></label><button type="button" className={midiLearnArmed ? "active" : ""} disabled={!audioRunning || !midiMapAvailable} title={!midiMapAvailable ? "Add a Core MIDI-Map module first" : !audioRunning ? "Start audio to receive Web MIDI" : "Move the next MIDI CC to create a mapping"} onClick={() => onArmMidiLearn(selectedLearnParamId)}>{midiLearnArmed ? "Waiting for CC…" : "Map next MIDI CC"}</button></div>}
