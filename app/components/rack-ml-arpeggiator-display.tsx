...
export function RackMlArpeggiatorDisplay({
  ...
  return (
    ...
    >
      <rect className="frame" x=".25" y=".25" width={width - 0.5} height={height - 0.5} />
      {horizontalRows.map((row) => (
        <line key={`row-${row}`} x1="0" x2={width} y1={row * rowHeight} y2={row * rowHeight} />
      ))}
      {Array.from({ length: activeChannels }, (_, channel) => {
        const offset = 1 + channel * 3,
          order = Math.max(0, Math.min(1, Math.round(values?.[offset] ?? 0))),
