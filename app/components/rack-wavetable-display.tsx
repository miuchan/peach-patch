...
export const RackWavetableDisplay = memo(function RackWavetableDisplay({
  ...
  useEffect(() => {
    ...
    const top = 18,
      ...
      samples = values.slice(3, 132);
    context.beginPath();
    for (let index = 0; index < samples.length; index++) {
      const px = 4 + ((width - 8) * index) / (samples.length - 1),
        sample = Number.isFinite(samples[index]) ? Math.max(-1, Math.min(1, samples[index])) : 0,
        py = top + plotHeight * (0.5 - 0.5 * sample);
