...
export function RackTapestryDisplay({
  ...
  const gradientId = `tapestry-waveform-${useId().replace(/:/g, "")}`,
    ...
    splices = Array.from({ length: spliceCount }, (_, index) =>
      bounded(values?.[5 + index] ?? 0, 0, 1),
    ),
    ...
    hoveredSplice =
      hoverX === null
        ? -1
        : splices.findIndex((marker) => Math.abs(marker - hoverX / width) <= hitTolerance),
    barWidth = 2.5,
    barGap = 1,
    barSpacing = barWidth + barGap,
    centerY = height * 0.5,
    maxBarHeight = height * 0.45;
    ...
    >
      <defs>
        <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
          ...
          <stop
            offset="1"
            stopColor={`rgb(${Math.round(red * 0.7)},${Math.round(green * 0.7)},${Math.round(blue * 0.7)})`}
            stopOpacity=".71"
          />
        </linearGradient>
      </defs>
      ...
      {present &&
        Array.from({ length: bins }, (_, bin) => {
          const peak = Math.max(0, values?.[waveOffset + bin] ?? 0),
            barHeight = Math.max(2, Math.pow(peak, 0.7) * maxBarHeight),
            barX = bin * barSpacing,
          ...
          return (
            <g key={bin}>
              ...
              {barHeight > 3 ? (
                <>
                  <circle
                    cx={barX + barWidth * 0.5}
                    cy={centerY - barHeight}
                    r={barWidth * 0.5}
                    fill={`rgba(${red},${green},${blue},${hovered ? 1 : 0.86})`}
                  />
                  <circle
                    cx={barX + barWidth * 0.5}
                    cy={centerY + barHeight}
                    r={barWidth * 0.5}
                    fill={`rgba(${Math.round(red * 0.7)},${Math.round(green * 0.7)},${Math.round(blue * 0.7)},${hovered ? 1 : 0.78})`}
                  />
                </>
              ) : null}
              {hovered ? (
      ...
      {present ? (
        <line
          ...
          x2={width}
          y2={centerY}
          stroke="rgba(100,100,120,.235)"
          strokeWidth=".5"
        />
      ) : null}
      {present &&
        splices.map((marker, index) => (
          <line
            key={index}
            x1={marker * width}
            y1="0"
            x2={marker * width}
            y2={height}
            stroke={index === currentSplice ? "#ffc832" : "#c89632"}
            strokeWidth={index === currentSplice ? 2 : 1}
          />
        ))}
      {present ? (
        <line
      ...
      {present && hoverX !== null ? (
        <>
          <line
            ...
            x2={hoverX}
            y2={height}
            stroke={hoveredSplice >= 0 ? "rgba(255,100,100,.706)" : "rgba(100,255,100,.588)"}
            strokeWidth={hoveredSplice >= 0 ? 3 : 2}
          />
          <path
            d={`M${hoverX},0 L${hoverX - 5},-5 L${hoverX + 5},-5 Z`}
            fill={hoveredSplice >= 0 ? "rgba(255,100,100,.784)" : "rgba(100,255,100,.784)"}
          />
        </>
      ) : null}
      <rect
